...
export default function ProjectEditors() {
  ...
  const selectedReviewFileIds = location.state?.selectedReviewFileIds || [];
  console.log("Review file IDs received:", selectedReviewFileIds);
  ...
  const toggleSelect = (id) => {
    setSelected((prev) =>
      prev.includes(id)
        ? prev.filter((u) => u !== id)
        : [id]  // only ONE reviewer allowed
    );
  };

  const handleAssignReviewFiles = async () => {
    if (selected.length !== 1) {
      alert("Select exactly ONE reviewer.");
      return;
    }

    const payload = {
      reviewer_id: selected[0],
      file_ids: selectedReviewFileIds
    };

    console.log("Sending to backend:", payload);

    try {
      await axios.post(`${apiBase}/reviews/link`, payload);
      alert("Review files assigned successfully!");
      navigate("/admin/ongoingprojects");
    } catch (err) {
      console.error("Error assigning review files:", err);
      alert("Failed to assign review files.");
    }
  };
  ...
  return (
    <div className="p-6 max-w-5xl mx-auto">
            ...
            <tbody>
              {editors.map((editor, idx) => (
                <tr key={idx} className="border-t">
                  ...
                  <td className="p-3">{editor.user_id}</td>
      ...
      {selected.length > 0 && (
        <div className="mt-6 flex justify-end">
          <button
            onClick={handleAssignReviewFiles}
            className="px-6 py-3 bg-green-600 text-white rounded-lg hover:bg-green-700 transition shadow"
          >
            Assign Review File
          </button>
